...
class Prefixes:
    ...
    def __set_prefixes_and_labels(
        ...
        if num_demos == 0:
            max_token_len_per_demo = max_token_len
            len_prefix_per_demo = len(tokenizer(prefix_narrative)["input_ids"])
        else:
            max_token_len_per_demo = max_token_len // num_demos
            len_prefix_per_demo = len(tokenizer(prefix_narrative)["input_ids"]) // num_demos

        avg_samp_size = (
            (max_token_len_per_demo)
